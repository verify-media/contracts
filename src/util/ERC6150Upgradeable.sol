...
abstract contract ERC6150Upgradeable is ERC721Upgradeable, IERC6150 {
    mapping(uint256 => uint256) internal _parentOf;
    mapping(uint256 => uint256[]) internal _childrenOf;
    mapping(uint256 => uint256) internal _indexInChildrenArray;

    // constructor(string memory name_, string memory symbol_) ERC721(name_, symbol_) {}
    function __ERC6150_init(string memory name_, string memory symbol_) internal onlyInitializing {
        __ERC6150_init_unchained(name_, symbol_);
    }

    function __ERC6150_init_unchained(string memory name_, string memory symbol_) internal onlyInitializing {
    ...
    function _safeBatchMintWithParent(address to, uint256 parentId, uint256[] memory tokenIds) internal virtual {
        _safeBatchMintWithParent(to, parentId, tokenIds, new bytes[](tokenIds.length));
    }

    function _safeBatchMintWithParent(address to, uint256 parentId, uint256[] memory tokenIds, bytes[] memory datas)
        internal
        virtual
    {
    ...
    function _beforeMintWithParent(address to, uint256 parentId, uint256 tokenId) internal virtual {}

    function _afterMintWithParent(address to, uint256 parentId, uint256 tokenId) internal virtual {}
}
